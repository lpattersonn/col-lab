...
export default function BorrowItemsChat() {
    ...
    const SideBarChats = allChats.map((chat, index) => {
        if (userDetails?.id === chat?.acf?.requestor_id || userDetails?.id === chat?.acf?.participant_id) {
            ...
            if (array !== null) { 
                ...
                return (
                    <a href={`/borrow-items-chat/${chat.id}`} key={index} target="_self" titl={`Link to chat with ${chat?.acf?.requestor_name}`}>
                        ...
                        <div className='row'>
                            ...
                            <div className='col-lg-9 mentors-chat-item p-0'>
                                ...
                                <div className={`mentors-chat-item-body ${overFLow === false ? 'overflow-scroll' : 'overflow-hidden'}`}>
                                    <div class="collaboration-title">Borrow Item: {chatDetails?.acf?.request_title}</div>
                                    {conversation}
                                </div>
