...
export default function LearnCenterChat() {
    ...
    useEffect(() => {
        axios.get(`${process.env.REACT_APP_API_URL}/wp-json/wp/v2/learning-center-chat`, {
    ...
    const fetchData = async () => {
        try {
            const [allMentorRequests, singleChat, singleUserDetails, allComments] = await Promise.all([
                ...
                axios.get(`${process.env.REACT_APP_API_URL}/wp-json/wp/v2/learning-center-chat/${param1}`, {
                    headers: { Authorization: `Bearer ${userDetails?.token}` }
                }),
